...
import { renderPlanToGpuLayers, GPU_PREVIEW_ENABLED } from "@/lib/editor-v2/editor/gpu-preview-bridge";
...
export function CanvasStage() {
  const {
    ...
    elements, selectedElementId, selectElement, selectClip, updateElement,
    background, addElement, clips, pushHistory, transitions, tracks, settings,
    setPlayhead, playing, togglePlay, selectedClipIds,
  } = useEditor();
  ...
  const gpuCompositorRef = useRef<GpuCompositor | null>(null);
  ...
  // GPU preview (behind NEXT_PUBLIC_GPU_PREVIEW): mount/dispose the WebGL2
  // compositor once. The DOM path (ElementBody's <img>/<video>) stays fully
  // intact underneath when this is off — zero behavior change by default.
  useEffect(() => {
    if (!GPU_PREVIEW_ENABLED || !gpuCanvasRef.current) return;
    gpuCompositorRef.current = new GpuCompositor(gpuCanvasRef.current);
    return () => {
  ...
  // Re-draws the GPU canvas every time the render plan changes — the render
  // plan already recomputes on every playhead tick (playback.ts's RAF loop
  // feeds playhead → this useMemo), so this rides the same clock rather than
  // running its own RAF loop.
  useEffect(() => {
    const compositor = gpuCompositorRef.current;
    const canvas = gpuCanvasRef.current;
    if (!GPU_PREVIEW_ENABLED || !compositor || !canvas || !stageW || !stageH) return;
    if (canvas.width !== stageW) canvas.width = stageW;
    if (canvas.height !== stageH) canvas.height = stageH;
    const layers = renderPlanToGpuLayers(renderPlan.renderedItems, stageW, stageH);
    compositor.compositeFrame(
      layers,
      { width: stageW, height: stageH, framebuffer: null },
      { presentationTimestamp: playhead, timelineTimestamp: playhead, version: 1 },
    );
  }, [renderPlan, stageW, stageH, playhead]);
  ...
  const dragRef = useRef<{
    ...
    el: StageElement;
    hasMoved?: boolean;
    /** Other selected same-kind elements (via linked clips) to translate alongside this one. */
  ...
  const onElementPointerDown = (e: ReactPointerEvent, el: StageElement) => {
    ...
    // Shift/Ctrl/Cmd-click on the stage toggles this element's clip into a
    // multi-selection (mirrors the timeline's multi-select) — selection-only,
    // doesn't start a drag.
    if (clip && (e.shiftKey || e.metaKey || e.ctrlKey)) {
      selectClip(clip.id, true);
      return;
    }

    // If this element's clip is part of a multi-selection, keep the whole
    // selection intact (don't collapse to just this element) so dragging
    // moves every selected same-kind element together.
    const isGroupDrag = !!clip && state.selectedClipIds.length > 1 && state.selectedClipIds.includes(clip.id);
    if (!isGroupDrag) selectElement(el.id);
    if (clip?.frozen) {
    ...
    let groupOrigins: Record<string, { x: number; y: number }> | undefined;
    if (isGroupDrag) {
      groupOrigins = {};
      for (const id of state.selectedClipIds) {
        if (id === clip!.id) continue;
        const c = state.clips.find((x) => x.id === id);
        if (!c || c.kind !== clip!.kind || c.frozen || !c.elementId) continue;
        const t = state.tracks.find((tt) => tt.id === c.track);
        if ((t as { locked?: boolean } | undefined)?.locked) continue;
        const oEl = state.elements.find((x) => x.id === c.elementId);
        if (!oEl) continue;
        groupOrigins[oEl.id] = { x: oEl.x, y: oEl.y };
      }
    }

    dragRef.current = {
      id: el.id, mode: "move", startX: e.clientX, startY: e.clientY,
      rect: stageEl.getBoundingClientRect(), el, groupOrigins,
    };
  };
  ...
  const onPointerMove = (e: ReactPointerEvent) => {
    ...
    if (d.mode === "move") {
      ...
      // Translate the rest of a multi-selection by the same raw delta
      // (unsnapped — snapping only anchors the primarily-dragged element).
      if (d.groupOrigins) {
        for (const [gid, origin] of Object.entries(d.groupOrigins)) {
          const gEl = elements.find((x) => x.id === gid);
          if (!gEl) continue;
          updateElement(gid, {
            x: clamp(origin.x + dxPct, -gEl.w + 5, 100 - 5),
            y: clamp(origin.y + dyPct, -gEl.h + 5, 100 - 5),
          });
        }
      }
      return;
  ...
  const onPointerUp = () => {
    ...
    if (d && d.hasMoved) {
      ...
      const latestElements = useEditor.getState().elements;
      const core = useEditorCore.getState();
      const applyTransform = (elId: string) => {
        const finalEl = latestElements.find((x) => x.id === elId);
        if (!finalEl) return;
      ...
      applyTransform(d.id);
      if (d.groupOrigins) {
        for (const gid of Object.keys(d.groupOrigins)) applyTransform(gid);
      }
    }
  ...
  const onStageDrop = (e: React.DragEvent) => {
    ...
    if (created) {
      ...
      state.updateClip(created.id, {
        ...
        // Without the asset id, a global-library item dropped here saves with
        // src only; the next load's cross-project media guard can't resolve it
        // and silently strips the clip (timeline drop already passes it).
        mediaAssetId: m.mediaAssetId, sourceDuration: m.duration,
      });
    }
          ...
          >
            ...
            {renderPlan.renderedItems.map((item) => (
              <StageElementView
                ...
                stageH={stageH}
                selected={
                  item.element.id === selectedElementId ||
                  (selectedClipIds.length > 1 && selectedClipIds.includes(item.clip.id))
                }
                onPointerDown={(e) => onElementPointerDown(e, item.element)}
                onHandlePointerDown={(e, h) => onHandlePointerDown(e, item.element, h)}
                hideRaster={GPU_PREVIEW_ENABLED && (item.element.kind === "image" || item.element.kind === "video")}
              />
            ))}
            
            {/* Smart alignment guides — z-index above every clip layer (subtitle
                tops out at 100, see CLIP_LAYER_PRIORITY) so they're never
                hidden behind the element being dragged. */}
            {settings.showGuides && (
...
function StudioTextContent({
  ...
  if (el.studioOverlay === "subtitle") {
    ...
    return (
      <div ref={contentRef} style={{ width: "100%", ...studioSubtitleBoxStyle(scaleW, scaleH), ...studioSubtitleTextStyle(ss, scaleH) }}>
        {content}
      </div>
    );
  }
  ...
  if (el.studioOverlay === "title") {
    ...
    return (
      <div ref={contentRef} style={{ width: "100%", ...studioTitleBoxStyle(scaleW, scaleH), ...studioTitleTextStyle(ts, scaleH) }}>
        {el.text || "Text"}
      </div>
    );
  }
...
function StageElementView({
  item, stageW, stageH, selected, onPointerDown, onHandlePointerDown, hideRaster,
}: {
  item: CanvasRenderItem;
  stageW: number;
  stageH: number;
  ...
  /** True when the GPU canvas beneath is already painting this element's
   * pixels — the DOM node stays mounted (still owns drag/selection/handles)
   * but its own raster content is hidden so it doesn't double-render. */
  hideRaster?: boolean;
}) {
  const { element: el } = item;
  const contentRef = useRef<HTMLDivElement>(null);

  // Studio title/subtitle overlays are placed inside a fixed-height zone (e.g.
  // 16% of stage height) that's much taller than a single line of text, and
  // the text is vertically centered inside it — so the selection outline
  // (drawn from el.x/y/w/h) doesn't hug the actual visible pill. Measure the
  // real rendered content and tighten h/y to match it, keeping the visual
  // center fixed so nothing appears to jump.
  useEffect(() => {
    if (!el.studioOverlay) return;
    const node = contentRef.current;
    if (!node || !stageH) return;
    const sync = () => {
      const rect = node.getBoundingClientRect();
      const newH = (rect.height / stageH) * 100;
      if (!Number.isFinite(newH) || newH <= 0 || Math.abs(newH - el.h) < 0.15) return;
      const newY = el.y + (el.h - newH) / 2;
      useEditor.getState().updateElement(el.id, { h: newH, y: newY });
    };
    const ro = new ResizeObserver(sync);
    ro.observe(node);
    sync();
    return () => ro.disconnect();
  }, [el.studioOverlay, el.id, el.text, el.fontSize, el.fontFamily, el.fontWeight, el.letterSpacing, el.h, el.y, stageH]);
  ...
  const handlePos: Record<HandleId, string> = {
    ...
    tc: "-top-1 left-1/2 -translate-x-1/2 cursor-ns-resize",
    bc: "-bottom-1 left-1/2 -translate-x-1/2 cursor-ns-resize",
    lc: "top-1/2 -translate-y-1/2 -left-1 cursor-ew-resize",
    rc: "top-1/2 -translate-y-1/2 -right-1 cursor-ew-resize",
    rot: "",
  };
  ...
  if (el.studioOverlay) {
    return (
      <div
        ...
        style={{
          ...
          opacity,
          // While a transition is active the pair stacks by ROLE (incoming above
        // outgoing), not by clip kind — see CanvasRenderItem.zIndexOverride.
        zIndex: item.zIndexOverride ?? clipLayerZIndex(item.clip.kind),
    ...
    >
      <StudioTextContent item={item} scaleW={scaleW} scaleH={scaleH} contentRef={contentRef} />
        {selected && (
          <div className="absolute inset-0 pointer-events-none">
            <div className={`absolute inset-0 border-2 ${item.clip.frozen ? "border-sky-500/70" : "border-brand/70"} rounded-lg`} />
            {!item.clip.frozen && handles.map((p) => (
              <span
                key={p}
                onPointerDown={(e) => onHandlePointerDown(e, p)}
                className={`absolute ${handlePos[p]} size-2 bg-white border border-brand rounded-sm pointer-events-auto`}
              />
            ))}
          </div>
        )}
  ...
  return (
    ...
    >
      <ElementBody item={item} scale={elementScale} hideRaster={hideRaster} />
      {selected && (
        <div className="absolute inset-0 pointer-events-none">
          ...
          {!item.clip.frozen && (
            <>
              ...
              <span
                ...
                className="absolute -top-6 left-1/2 -translate-x-1/2 size-2 rounded-full border-2 border-brand bg-background pointer-events-auto cursor-grab"
              />
            </>
          )}
        </div>
...
function ElementBody({ item, scale, hideRaster }: {
  item: CanvasRenderItem; scale: number; hideRaster?: boolean;
}) {
  const el = item.element;
  ...
  if (el.kind === "text") {
    ...
    const absMs = absTime * 1000;
    let words = el.words ?? [];
    if ((!words || words.length === 0) && el.text && (el.animation === "karaoke" || el.animation === "highlight" || el.animation === "word_pop")) {
      const rawWords = el.text.trim().split(/\s+/).filter(Boolean);
      if (rawWords.length > 0) {
        const clipStartMs = (item.clip.start / PX_PER_SECOND) * 1000;
        const clipDurMs = (item.clip.width / PX_PER_SECOND) * 1000;
        const perWordMs = clipDurMs / rawWords.length;
        words = rawWords.map((w, idx) => ({
          word: w,
          startMs: Math.round(clipStartMs + idx * perWordMs),
          endMs: Math.round(clipStartMs + (idx + 1) * perWordMs),
        }));
      }
    }
  ...
  const borderStyle = el.stroke ? `${el.strokeWidth || 2}px solid ${el.strokeColor || "#000"}` : undefined;
  const strokeAttrs = el.stroke
    ? { stroke: el.strokeColor || "#000", strokeWidth: el.strokeWidth || 2 }
    : {};

  if (el.kind === "image") {
    const objectFitClass = el.fit === "contain" ? "object-contain" : "object-cover";
    return (
      <div className="w-full h-full box-border" style={{ boxShadow: borderStyle ? `inset 0 0 0 ${el.strokeWidth || 2}px ${el.strokeColor || "#000"}` : undefined }}>
        <img
          src={el.src || undefined}
          alt=""
          className={`w-full h-full ${objectFitClass}`}
          style={{ filter: filter === "none" ? undefined : filter, opacity: hideRaster ? 0 : undefined }}
          draggable={false}
        />
      </div>
    );
  ...
  if (el.kind === "video") {
    return (
      <div className="w-full h-full box-border" style={{ boxShadow: borderStyle ? `inset 0 0 0 ${el.strokeWidth || 2}px ${el.strokeColor || "#000"}` : undefined }}>
        <VideoBody src={el.src} filter={filter} item={item} fit={el.fit} hideRaster={hideRaster} />
      </div>
    );
  }
  if (el.kind === "rect") return <div className="w-full h-full" style={{ background: el.color, filter: filter === "none" ? undefined : filter, boxShadow: borderStyle ? `inset 0 0 0 ${el.strokeWidth || 2}px ${el.strokeColor || "#000"}` : undefined }} />;
  if (el.kind === "ellipse") return <div className="w-full h-full rounded-full" style={{ background: el.color, filter: filter === "none" ? undefined : filter, boxShadow: borderStyle ? `inset 0 0 0 ${el.strokeWidth || 2}px ${el.strokeColor || "#000"}` : undefined }} />;
  if (el.kind === "triangle") {
    return (
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full" style={{ filter: filter === "none" ? undefined : filter }}>
        <polygon points="50,4 96,96 4,96" fill={el.color} {...strokeAttrs} />
      </svg>
    );
  }
  if (el.kind === "shape") {
    ...
    return (
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full" style={{ filter: filter === "none" ? undefined : filter }}>
        ...
        {sType === "cloud" && (
          <path d="M 25,60 A 20,20 0 0,1 45,40 A 25,25 0 0,1 85,45 A 20,20 0 0,1 85,85 H 25 A 20,20 0 0,1 25,60 Z" fill={el.color} {...strokeAttrs} />
        )}
        {sType === "star" && (
          <polygon points="50,5 64,36 98,36 70,57 81,91 50,70 19,91 30,57 2,36 36,36" fill={el.color} {...strokeAttrs} />
        )}
        {sType === "arrow" && (
          <polygon points="10,35 60,35 60,15 90,50 60,85 60,65 10,65" fill={el.color} {...strokeAttrs} />
        )}
        {sType === "pentagon" && (
          <polygon points="50,5 95,38 78,92 22,92 5,38" fill={el.color} {...strokeAttrs} />
        )}
        {sType === "hexagon" && (
          <polygon points="50,5 90,28 90,72 50,95 10,72 10,28" fill={el.color} {...strokeAttrs} />
        )}
        {sType === "heart" && (
          <path d="M50,30 C50,30 50,15 35,15 C18,15 18,37 18,37 C18,58 50,82 50,85 C50,82 82,58 82,37 C82,37 82,15 65,15 C50,15 50,30 50,30 Z" fill={el.color} {...strokeAttrs} />
        )}
        {sType === "bubble" && (
          <path d="M10,15 h80 v50 H55 L35,85 V65 H10 Z" fill={el.color} {...strokeAttrs} />
        )}
      </svg>
    );
  }
...
function VideoBody({ src, filter, item, fit, hideRaster }: {
  src?: string; filter: string; item: CanvasRenderItem; fit?: "cover" | "contain"; hideRaster?: boolean;
}) {
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const mediaSrc = item.clip.src || src;

    let el = mediaPool.get(clipId);
    if (!el && mediaSrc) {
      el = mediaPool.acquire(clipId, "video", mediaSrc);
    }
    if (!el || el.tagName !== "VIDEO") return;

    const container = containerRef.current;

    // Only append if the element isn't already inside this container.
    // Moving an active <video> element between DOM parents forces Chromium to
    // reset the media decoder, causing black flashes and frozen frames.
  ...
  }, [item.clip.id, item.clip.src, src, fit]);

  useEffect(() => {
    if (elRef.current) {
      elRef.current.style.filter = filter === "none" ? "none" : filter;
      elRef.current.style.opacity = hideRaster ? "0" : "1";
    }
  }, [filter, hideRaster]);
