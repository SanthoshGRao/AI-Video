...
export function TopNav({
  ...
  return (
    <nav
      className={cn(
        "relative h-13 shrink-0 z-40 flex items-center justify-between px-4 py-2 bg-gradient-to-r from-[oklch(0.26_0.03_280)] via-[oklch(0.24_0.025_270)] to-[oklch(0.23_0.02_255)] border-b border-white/10 shadow-lg shadow-black/30",
        // Same reserved-space fix as the dashboard TopNav: don't let Export/
        // Next render under the custom hover-revealed window controls.
        isDesktop && "pr-[140px]"
      )}
    ...
    >
      ...
      {/*
        Punches the window-control strip out of this nav's drag region.

        The nav marks itself `-webkit-app-region: drag`, and `pr-[140px]`
        only reserves visual space — the element still spans the full width,
        so the drag region covered the minimize/maximize/close buttons.
        Draggable regions are resolved by Chromium at the OS level and take
        precedence over DOM hit-testing, so those buttons were painted on top
        (visible, hover styles and all) but never received clicks.

        DesktopWindowControls does set `no-drag` on itself, but it is mounted
        globally in app/layout.tsx — a *sibling* subtree, not a descendant of
        this drag region — and cross-subtree no-drag subtraction is exactly
        the case Chromium does not handle reliably. This spacer is a
        descendant, which is the same mechanism the nav's own left/right
        button groups already rely on, so the subtraction is dependable.

        The dashboard's TopNav avoids the problem a different way (it ends at
        `right-[140px]`), which isn't available here because this nav is a
        flow-layout flex child rather than absolutely positioned.
      */}
      {isDesktop && (
        <div
          aria-hidden="true"
