  ...
  return withSerializableRetry(() =>
    prisma.$transaction(
      async (tx) => {
        ...
        // `settings.subtitleTrackId` is server-owned metadata that only the
        // dedicated /subtitles route ever pins (it gates which language
        // track's style may restyle the timeline's shared subtitle clips —
        // see subtitles/route.ts). The editor-v2 store has no UI to change
        // this value; it only loads it at mount time and echoes it straight
        // back on every save. Without this guard, a generic editor
        // save/autosave — built from whatever was in memory when the editor
        // tab was opened — silently overwrites a pin the Subtitle Studio set
        // afterwards, reintroducing "last styled track wins".
        const existingSettings = existing?.settings && typeof existing.settings === "object"
          ? existing.settings as Record<string, unknown>
          : null;
        const writePayload: typeof payload = existingSettings && "subtitleTrackId" in existingSettings
          ? {
              ...payload,
              settings: {
                ...(payload.settings as Record<string, unknown>),
                subtitleTrackId: existingSettings.subtitleTrackId,
              } as Prisma.InputJsonValue,
            }
          : payload;

        if (existing && isAutosave && !bumpVersion && existing.isAutosave) {
          return tx.timeline.update({ where: { id: existing.id }, data: writePayload });
        }
        if (existing && !bumpVersion && !isAutosave) {
          return tx.timeline.update({ where: { id: existing.id }, data: { ...writePayload, isAutosave: false } });
        }
