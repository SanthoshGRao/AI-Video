...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    if (customStyle !== undefined && customStyle) {
      ...
      // The timeline's subtitle clips are shared across language tracks, so
      // only the ACTIVE track's style may restyle them. Without this gate,
      // styling track A and then merely touching track B's style seconds
      // later overwrote the timeline with B's fonts — the user's customization
      // on A looked like it never saved.
      const activeTrackId = (timeline?.settings as { subtitleTrackId?: string } | null)?.subtitleTrackId;
      const wasUnset = !activeTrackId || activeTrackId === "force-rebuild";
      const isActiveTrack = wasUnset || activeTrackId === trackId;
      if (isActiveTrack && timeline?.clips && typeof timeline.clips === "object") {
        ...
        // Pin this as the active track the moment it's styled if nothing was
        // pinned yet — otherwise `subtitleTrackId` never gets initialized
        // (the GET route only sets it on an explicit track-switch click), so
        // the gate above stayed permanently "unset => always active" and
        // whichever track a multi-language project styled LAST silently
        // restyled the timeline's shared subtitle clips out from under
        // whatever the user was actually looking at.
        const settingsUpdate = timeline.settings && typeof timeline.settings === "object"
          ? { ...(timeline.settings as Record<string, any>) }
          : {};
        if (wasUnset && settingsUpdate.subtitleTrackId !== trackId) {
          settingsUpdate.subtitleTrackId = trackId;
          changed = true;
        }
        if (changed) {
          await prisma.timeline.update({
            where: { id: timeline.id },
            data: {
              clips: clips as Prisma.InputJsonValue,
