...
{
  "translations": [
    { "id": "cue_id", "text": "translated text" }
  ]
}`;
    ...
    let object: z.infer<typeof translationSchema>;
    try {
      ...
      console.warn(
        `[TRANSLATE_SUBTITLES] GPT-4o failed, attempting Gemini fallback:`,
        err instanceof Error ? err.message : String(err)
      );
      if (hasGeminiTextConfigured()) {
        const text = await generateGeminiText({ prompt, temperature: 0.7 });
        object = parseJsonFromText(text, translationSchema);
      } else {
        throw err;
      }
    }
