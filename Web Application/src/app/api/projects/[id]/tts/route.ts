...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const { scriptVersionId, text, voiceType = "Charon", languageCode, styleId, customInstructions } = body;

    const voice = resolveVoicePersona(voiceType, languageCode);
    const { audioBuffer, durationMs, sync, styleLabel } = await synthesizeVoiceover(text, voice, {
      styleId,
      customInstructions,
    });
