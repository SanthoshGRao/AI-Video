...
function parseSettings(raw: unknown): TimelineSettings {
  ...
  return {
    ...
    aspectRatio,
    // Sticky active subtitle track — the subtitles PUT route only lets the
    // active track restyle timeline clips, so dropping this here would let
    // any language track's style clobber the clips after an editor save.
    subtitleTrackId: o.subtitleTrackId ? String(o.subtitleTrackId) : undefined,
    projectMediaIds: Array.isArray(o.projectMediaIds)
      ? (o.projectMediaIds as unknown[]).map(String)
      : undefined,
  };
}
