...
class MediaEngine {
  ...
  /** URLs with a register() in flight — keeps ensureRegistered() idempotent. */
  ...
  /**
   * Register a URL only if it isn't already tracked.
   *
   * register() is called on upload, but media restored from a saved project
   * (or produced by the AI pipeline) never passes through the upload path —
   * so no AssetRecord existed and the timeline had no thumbnail to draw.
   * Callers can fire this freely on every render; it de-dupes by URL.
   */
  ensureRegistered(input: RegisterInput): void {
    const url = input.originalUrl;
    if (!url || typeof document === "undefined") return;
    if (this.registering.has(url)) return;
    if (assetManager.all().some((a) => a.originalUrl === url)) return;
    this.registering.add(url);
    void this.register(input).finally(() => this.registering.delete(url));
  }
  ...
  private async thumbnailMain(
    ...
    const captureFrame = (url: string, useCors: boolean): Promise<{ blob: Blob; width: number; height: number } | null> =>
      new Promise((resolve) => {
        const v = document.createElement("video");
        let done = false;
        const finish = (val: { blob: Blob; width: number; height: number } | null) => {
          if (done) return;
          done = true;
          clearTimeout(timer);
        ...
        const seekAndCapture = () => {
          ...
          const capture = () => {
            try {
              ...
              c.toBlob((b) => { b ? finish({ blob: b, width: w, height: h }) : finish(null); }, "image/webp", 0.8);
            } catch { finish(null); }
          };
          v.onseeked = capture;
        ...
        v.onloadedmetadata = seekAndCapture;
        v.onloadeddata = seekAndCapture;
        v.onerror = () => finish(null);
        v.src = url;
        try { v.load(); } catch { /* noop */ }
      });

    const result = await captureFrame(a.originalUrl, true);
    if (result) return result;
    // CORS attempt failed (tainted canvas or load error) — retry without crossOrigin
    return captureFrame(a.originalUrl, false);
  }
