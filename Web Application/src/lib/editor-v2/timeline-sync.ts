...
function transitionType(kind: Transition["kind"]) {
  ...
  // "dissolve" used to map to "blur", which the ffmpeg exporter renders as
  // xfade=fadeblack — so picking Dissolve in the editor exported a dip to
  // BLACK while the preview showed a cross-dissolve. Fade and dissolve are the
  // same cross-dissolve in the preview (transition-style.ts), so they map to
  // the same wire type.
  return "fade";
}
...
function elementStyle(el?: StageElement) {
  ...
  return {
    ...
    // The export renderer's generic "text" branch (render-doc-adapter.ts)
    // reads these two exact keys directly rather than the raw fields above
    // (which shape/image/video items and reload use) — keep both in sync.
    WebkitTextStrokeColor: el.stroke ? (el.strokeColor || "#000") : undefined,
    WebkitTextStrokeWidth: el.stroke ? `${el.strokeWidth || 1}px` : undefined,
...
export function buildTimelineSaveBody(input: {
  ...
  fps: number;
  isAutosave?: boolean;
  /** MediaAsset ids currently in the project-media panel — persisted in
   * settings so global-library imports survive a reload even before they're
   * placed on the timeline. */
  projectMediaIds?: string[];
  /** Sticky active subtitle track (settings.subtitleTrackId) — must round-trip
   * through editor saves or the subtitles route loses its restyle gate. */
  ...
  for (const clip of input.clips) {
    ...
    clips[clip.id] = {
      ...
      properties: {
        ...
        // export — and reload — honour the exact mute/volume/fade the user
        // set in the editor.
        muted: clip.audio?.muted ?? false,
        volume: clip.audio?.volume ?? 1,
        audioFadeIn: clip.audio?.fadeIn ?? 0,
        audioFadeOut: clip.audio?.fadeOut ?? 0,
        audioSolo: clip.audio?.solo ?? false,
        frozen: !!clip.frozen,
        // Original source-media duration (seconds), so a reload can still
        // cap how far a video/audio clip's trim handle can stretch.
        sourceDuration: clip.sourceDuration,
        // "ai" vs "user" — hydrateProject() on the next load uses this to
        // drop and refresh AI-authored title/subtitle clips against the
        // latest DB facts/captions while leaving user-added clips alone.
        source: clip.source ?? "user",
...
export async function saveEditorTimeline(projectId: string, isAutosave = false) {
  ...
  const body = buildTimelineSaveBody({
    ...
    isAutosave,
    subtitleTrackId: state.subtitleTrackId ?? undefined,
    projectMediaIds: state.projectMedia
...
function styleToElement(style: Record<string, unknown> | undefined, el: StageElement): StageElement {
  ...
  return {
    ...
    // `style.opacity` is already on the same 0-100 scale as `el.opacity` (see
    // elementStyle() above) — it must NOT be re-scaled here. `el.opacity` was
    // already correctly derived from the top-level 0-1 `properties.opacity`
    // before this function runs, so this only needs to pass a real value through.
    opacity: typeof style.opacity === "number" ? style.opacity : el.opacity,
    ...
    effect: (style.effect as StageElement["effect"]) ?? el.effect,
  };
}
...
export function timelineDocumentToEditorState(doc: TimelineDocument): {
  ...
  for (const clip of Object.values(doc.clips)) {
    ...
    const clipSource: "ai" | "user" = props.source === "ai" ? "ai" : "user";
    ...
    clips.push({
      ...
      audio: {
        ...
        volume: typeof props.volume === "number" ? props.volume : 1,
        fadeIn: typeof props.audioFadeIn === "number" ? props.audioFadeIn : 0,
        fadeOut: typeof props.audioFadeOut === "number" ? props.audioFadeOut : 0,
        solo: typeof props.audioSolo === "boolean" ? props.audioSolo : false,
      },
      frozen: props.frozen === true,
      sourceDuration: typeof props.sourceDuration === "number" ? props.sourceDuration : undefined,
    });
  }
  ...
  return {
    ...
    aspect: aspect.id,
    fps,
    subtitleTrackId: doc.settings.subtitleTrackId,
  };
}
