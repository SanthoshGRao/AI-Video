...
/** Shape written by POST /api/projects/[id]/facts/align — facts already
 *  pinned to the subtitle cue where they are spoken. */
export type AlignedFact = {
  id?: string;
  text: string;
  category?: string;
  startMs: number;
...
export class FactOverlayBuilder {
  ...
  async buildFactTrack(
    facts: ExtractedFacts | AlignedFact[] | null,
    ...
    // Extract fact mentions with timings. Facts that already went through
    // cue alignment (POST /facts/align) carry their own start/end and are
    // used verbatim — re-deriving them from keyword search would move the
    // overlay off the line that is actually being spoken.
    const rawTimings = Array.isArray(facts)
      ? this.timingsFromAlignedFacts(facts)
      : this.extractFactTimings(facts, cues);

    if (rawTimings.length === 0) {
      return { track: null, factElements: [] };
    }

    // Each fact holds the screen until the next one starts, and the last runs
    // to the end of the video — so the title track spans the video the same
    // way the subtitle track does instead of blinking a keyword-length card.
    const factTimings = layoutFactTimings(rawTimings, {
      totalDurationMs: totalDurationMs || cues[cues.length - 1]?.endMs,
    });
  ...
  /**
   * Pass through facts that already carry cue-aligned timings.
   */
  private timingsFromAlignedFacts(facts: AlignedFact[]): FactTiming[] {
    return facts
      .filter((f) => typeof f?.text === "string" && f.text.trim() && Number.isFinite(f.startMs))
      .map((f) => ({
        factText: f.text.trim(),
        startMs: f.startMs,
        endMs: Number.isFinite(f.endMs) ? f.endMs : f.startMs,
        category: f.category ?? this.categorizeFact(f.text),
      }));
  }
