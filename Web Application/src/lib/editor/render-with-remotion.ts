...
import { renderMedia, type FfmpegOverrideFn } from "@remotion/renderer";
...
import { runFfmpeg, pickVideoEncoder, videoEncodeArgs } from "@/lib/editor/ffmpeg";
...
async function createSilentVideoForRemotion(
  ...
  // Write to a temp name and rename on completion so a concurrent export of
  // the same project can't observe (or read) a partially-written file.
  const partialPath = `${outputPath}.partial-${process.pid}.mp4`;

  try {
    // Stream-copy the video track (no decode/encode) — this is a pure
    // container remux to drop audio, not a re-render, so it's ~instant
    // regardless of clip length. Falls back to a real re-encode below only
    // if the source codec/container can't be copied as-is.
    await runFfmpeg([
      "-i",
      inputPath,
      "-an",
  ...
  if (fs.existsSync(outputPath)) {
    // Another concurrent export finished first — keep its output, discard ours.
...
export async function renderProjectWithRemotion(options: {
  ...
  // don't clobber each other's output file in the shared tempDir.
  ...
  const outputLocation = path.join(tempDir, `remotion-output-${runId}.${format}`);
  ...
  // Remotion's own `hardwareAcceleration` option only ever engages on macOS
  // (VideoToolbox) — on Windows it silently stays on libx264. `ffmpegOverride`
  // is the only hook into the actual stitch-step ffmpeg invocation, so we use
  // it to swap in whichever hardware H.264 encoder actually works on this
  // machine (falls back to libx264 untouched if none do).
  const hwEncoder = format === "webm" ? null : await pickVideoEncoder();
  const ffmpegOverride: FfmpegOverrideFn | undefined =
    hwEncoder && hwEncoder !== "libx264"
      ? ({ type, args }) => {
          if (type !== "pre-stitcher") return args;
          const codecIndex = args.indexOf("-c:v");
          if (codecIndex === -1) return args;
          const next = [...args];
          next.splice(codecIndex, 2, ...videoEncodeArgs(hwEncoder));
          // Hardware encoders don't accept x264's `-crf` — Remotion always
          // appends one (default CRF 18) after the codec args.
          const crfIndex = next.indexOf("-crf");
          if (crfIndex !== -1) next.splice(crfIndex, 2);
          return next;
        }
      : undefined;
