...
import { type VoicePersonaConfig, resolveSpeakingInstructions } from "@/lib/tts/voices";
...
export type VoiceStyleOptions = {
  /** Built-in style preset id (see STYLE_PRESETS in voices.ts) */
  styleId?: string | null;
  /** Freeform director's-notes text, ad-hoc or loaded from a saved preset. Takes priority over styleId. */
...
export async function synthesizeVoiceover(
  ...
  styleOptions?: VoiceStyleOptions
  ...
  console.log(`[VOICEOVER_PIPELINE] Starting TTS Pipeline (single-call, consistent voice)`, {
    ...
    styleId: styleOptions?.styleId,
    hasCustomInstructions: !!styleOptions?.customInstructions?.trim(),
  });

  const { full: speakingInstructions, condensed: condensedSpeakingInstructions, styleLabel } =
    resolveSpeakingInstructions({
      voice,
      styleId: styleOptions?.styleId,
      customInstructions: styleOptions?.customInstructions,
      pitch: styleOptions?.pitch,
      pace: styleOptions?.pace,
      emotion: styleOptions?.emotion,
      energy: styleOptions?.energy,
    });
  ...
  return { audioBuffer, durationMs, sync, ttsProvider: providerUsed, styleLabel };
}
