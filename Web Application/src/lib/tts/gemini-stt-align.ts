...
export async function alignWithGeminiStt(
  ...
  if (!hasGeminiAccessConfigured()) throw new Error("Google AI API key not configured for Gemini STT.");
...
- Timestamps must be precise — align word boundaries carefully.${referenceBlock}`;
  ...
  const { url, headers } = geminiEndpoint(`v1beta/models/${getSttModel()}:generateContent`);
