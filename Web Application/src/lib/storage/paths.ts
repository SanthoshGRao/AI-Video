...
/**
 * Absolute path to /storage. Defaults to a folder at the project root, but can be
 * overridden (e.g. by the desktop app, which points this at Electron's userData
 * directory since the installed app's cwd isn't reliably writable).
 *
 * `process.cwd()` is inlined directly in the path.join() call (rather than via
 * the PROJECT_ROOT indirection above) because Next's output-file tracer only
 * recognizes this exact `path.join(process.cwd(), 'literal')` shape as scoped
 * to a subfolder — routing it through an intermediate constant makes the
 * tracer treat the whole project root as reachable and copy it wholesale into
 * `output: "standalone"` builds.
 */
export const STORAGE_ROOT =
  process.env.APP_STORAGE_ROOT || path.join(process.cwd(), "storage");
