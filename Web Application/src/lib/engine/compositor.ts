...
import type { TransitionFrameStyle } from "@/lib/editor-v2/transition-style";
...
export class Compositor {
  ...
  public async renderFrame(
    ...
    transitionStylesMap?: Record<string, TransitionFrameStyle>,
  ) {
    ...
    for (const item of activeItems) {
      ...
      // Active-transition overrides, computed by the caller from the shared
      // transitionFrameStyles() so export matches the editor's live preview
      // for every transition kind (fade/dissolve/slide/wipe/zoom/flip).
      const transitionStyle = transitionStylesMap?.[item.id];
      if (transitionStyle) {
        this.ctx.globalAlpha = opacity * transitionStyle.opacity;
        if (transitionStyle.translateXPercent != null) {
          this.ctx.translate((transitionStyle.translateXPercent / 100) * drawW, 0);
        }
        if (transitionStyle.scalePercent != null) {
          const s = transitionStyle.scalePercent / 100;
          this.ctx.translate(cx, cy);
          this.ctx.scale(s, s);
          this.ctx.translate(-cx, -cy);
        }
        if (transitionStyle.rotateYDeg != null) {
          // Canvas has no true 3D transform; approximate the flip with the
          // standard horizontal-foreshortening trick.
          const foreshorten = Math.cos((transitionStyle.rotateYDeg * Math.PI) / 180);
          this.ctx.translate(cx, cy);
          this.ctx.scale(foreshorten, 1);
          this.ctx.translate(-cx, -cy);
        }
        if (transitionStyle.clipInsetPercent) {
          const { left: insetLeft, right: insetRight } = transitionStyle.clipInsetPercent;
          const clipX = drawX + (insetLeft / 100) * drawW;
          const clipW = drawW * (1 - (insetLeft + insetRight) / 100);
          this.ctx.beginPath();
          this.ctx.rect(clipX, drawY, Math.max(0, clipW), drawH);
          this.ctx.clip();
        }
      }
      ...
      if (item.type === "video" || item.type === "image") {
          ...
          if (src) {
            ...
            if (asset && (asset.type === "video" || asset.type === "image")) {
              try {
                ...
                // Video items never reach here — export-player.tsx filters
                // them out before calling renderFrame() and draws video via
                // Remotion's <OffthreadVideo> instead (frame-accurate ffmpeg
                // extraction, not a seeked shared <video> element).
                const mediaEl = asset.element as HTMLImageElement;
                ...
                if (details.stroke) {
                  const strokeColor = (details.strokeColor as string) || "#000000";
                  const strokeWidth = Number(details.strokeWidth) || 2;
        ...
        try {
          ...
          const stroke = details.stroke
            ? { color: (details.strokeColor as string) || "#000000", width: Number(details.strokeWidth) || 2 }
            : undefined;
          this.drawShape(shapeType, color, drawX, drawY, drawW, drawH, cx, cy, stroke);
        } catch (err) {
  ...
  private drawShape(
    ...
    this.ctx.strokeStyle = stroke?.color || color || "#ffffff";

    if (shapeType === "rect" || shapeType === "rectangle") {
      this.ctx.fillRect(left, top, width, height);
      if (stroke) {
        this.ctx.lineWidth = stroke.width;
        this.ctx.strokeRect(left + stroke.width / 2, top + stroke.width / 2, width - stroke.width, height - stroke.width);
      }
      ...
      if (stroke) {
        this.ctx.lineWidth = stroke.width;
        this.ctx.beginPath();
        this.ctx.ellipse(cx, cy, Math.max(0, width / 2 - stroke.width / 2), Math.max(0, height / 2 - stroke.width / 2), 0, 0, 2 * Math.PI);
        this.ctx.stroke();
      }
      ...
      if (pathStr) {
        ...
        this.ctx.fill(path);
        if (stroke) {
          // Path coordinates are in a 0–100 unit box scaled to (width, height);
          // unscale the line width so it reads as a consistent pixel thickness.
          const avgDim = Math.max(1, (width + height) / 2);
          this.ctx.lineWidth = (stroke.width / avgDim) * 100;
