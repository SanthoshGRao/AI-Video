...
import { roundFrameTime, type MediaTime, TICKS_PER_SECOND } from "@/wasm";
...
function elementTypeFromDrag({
	...
	switch (dragData.type) {
		...
		case "effect":
			return "effect";
		case "transition":
			return "video" as any; // Map transition to video to allow dropping on video tracks
		case "media":
...
export class DragDropController {
	...
	private executeTransitionDrop({
		...
		// Find the closest junction between two clips on this track near target.xPosition
		const clips = track.elements;
		let closestA: typeof clips[0] | null = null;
		let closestB: typeof clips[0] | null = null;
		let minDistance = Infinity;

		for (let i = 0; i < clips.length - 1; i++) {
			const a = clips[i];
			const b = clips[i + 1];
			// The junction is where clip `a` ends and clip `b` starts
			// They don't have to perfectly align, but typically they are adjacent
			const junctionTime = a.startTime + a.duration;
			const distance = Math.abs(junctionTime - target.xPosition);
			
			// If within 2 seconds of drop target
			if (distance < minDistance && distance < 2 * TICKS_PER_SECOND) {
		...
		if (closestA && closestB) {
			this.config.addTransition({
				...
				durationMs: dragData.durationMs,
			});
		}
	}
