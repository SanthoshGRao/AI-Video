...
const Header = () => {
  const { duration, fps, scale, playerRef, activeIds, playbackState, setPlaybackState } = useStore();
  ...
  const seekBySeconds = (deltaSec: number) => {
    if (!playerRef?.current) return;
    const frame = getSafeCurrentFrame(playerRef);
    const next = Math.max(0, frame + deltaSec * fps);
      ...
      >
        ...
        >
          <div className="flex px-2">
            ...
            >
              <Copy size={14} />
              <span className="hidden lg:block">Duplicate</span>
            </Button>
          </div>
      ...
      <div className="flex lg:border-l pl-4 pr-2">
        ...
        <Slider
          ...
          onValueCommit={(value: number[]) => {
            const zoom = getZoomByIndex(value[0] ?? localValue);
