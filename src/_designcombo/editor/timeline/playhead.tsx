...
const Playhead = ({ scrollLeft }: { scrollLeft: number }) => {
  ...
  const { playerRef, fps, scale, playbackState, duration } = useStore();
  ...
  const handleMouseMove = (
    ...
    if (isDragging) {
      ...
      const newPosition = Math.max(0, dragStartPosition + delta);

      const time = Math.max(0, Math.min(unitsToTimeMs(newPosition, scale.zoom), duration));
  ...
  // requestAnimationFrame sync loop for 60 FPS timeline updates during active playback
  useEffect(() => {
    let animationFrameId: number;

    const updatePosition = () => {
      const player = playerRef?.current;
      if (player && playheadRef.current) {
        const frame = player.getCurrentFrame();
        const pos = timeMsToUnits((frame / fps) * 1000, scale.zoom) - scrollLeft;
        playheadRef.current.style.left = `${timelineOffsetX + TIMELINE_OFFSET_CANVAS_LEFT + pos}px`;
        
        // Directly update the timer text in the header to bypass React render overhead
        const timeEl = document.getElementById("video-current-time");
        if (timeEl) {
          timeEl.innerText = frameToTimeString({ frame }, { fps });
          timeEl.setAttribute("data-current-time", String(frame / fps));
        }
      }
  ...
  }, [playbackState, playerRef, fps, scale.zoom, scrollLeft, timelineOffsetX]);
