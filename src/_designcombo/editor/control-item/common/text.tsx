...
import ColorPicker from "@/_designcombo/color-picker";
...
const FontCase = ({ trackItem }: { trackItem: ITrackItem & any }) => {
  const [value, setValue] = useState(trackItem.details?.textTransform || "none");

  useEffect(() => {
    setValue(trackItem.details?.textTransform || "none");
  }, [trackItem.details?.textTransform]);

  const onChangeFontCase = (val: string) => {
    setValue(val);
    dispatch(EDIT_OBJECT, {
      payload: { [trackItem.id]: { details: { ...(trackItem.details || {}), 
            textTransform: val
           } } }
    });
  };
