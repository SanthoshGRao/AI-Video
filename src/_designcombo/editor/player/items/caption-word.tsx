...
export const CaptionWord: React.FC<CaptionWordProps> = ({
  ...
  const currentFrame = currentFrameOverride ?? playerFrame;
  const { start, end } = word;
  const startAtFrame = ((start + offsetFrom) / 1000) * fps;
  const endAtFrame = ((end + offsetFrom) / 1000) * fps;
  // Use >= for start to avoid gaps at exact frame boundaries.
  // Add 1-frame buffer to end to bridge micro-gaps between consecutive WhisperX words.
  const isActive = currentFrame >= startAtFrame && currentFrame < endAtFrame + 1;
