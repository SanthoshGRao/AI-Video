...
const useDragAndDrop = (onDragStateChange?: (isDragging: boolean) => void) => {
  ...
  const onDragEnter = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      e.preventDefault();
      try {
        const draggedData = useStore.getState().draggedItem;
        if (!draggedData) return;

        if (!Object.values(AcceptedDropTypes).includes(draggedData.type as AcceptedDropTypes))
          return;
  ...
  const onDrop = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      ...
      try {
        const draggedData = useStore.getState().draggedItem;
        if (draggedData) {
          handleDrop(draggedData as DraggedData);
        }
      } catch (error) {
