...
import { compareClipPaintOrder } from "@/lib/editor-v2/layer-priority";
import { transitionPairAtPlayhead } from "@/lib/editor-v2/transition-runtime";
...
function buildItem(
  ...
  const element: StageElement = {
    ...
    color: clip.color ?? baseElement.color,
  };
...
function transitionStyles(
  ...
  switch (kind) {
    ...
    case "slide":
      return {
        outgoing: { transform: `translateX(${-100 * p}%)`, opacity: 1 },
        incoming: { transform: `translateX(${100 * (1 - p)}%)`, opacity: 1 },
      };
    case "wipe":
      return {
        outgoing: { clipPath: `inset(0 ${100 * p}% 0 0)` },
        incoming: { clipPath: `inset(0 0 0 ${100 * (1 - p)}%)` },
      };
    case "zoom":
      return {
        outgoing: { transform: `scale(${1 + 0.4 * p})`, opacity: 1 - p, transformOrigin: "center" },
        incoming: { transform: `scale(${0.6 + 0.4 * p})`, opacity: p, transformOrigin: "center" },
      };
    case "flip":
      return {
        outgoing: { transform: `perspective(1000px) rotateY(${90 * p}deg)`, opacity: 1 - p, transformOrigin: "center" },
        incoming: { transform: `perspective(1000px) rotateY(${-90 * (1 - p)}deg)`, opacity: p, transformOrigin: "center" },
      };
    default:
...
export function buildCanvasRenderPlan(
  ...
  // Apply transitions: for each transition whose window covers the playhead,
  // overlap the outgoing + incoming clips on that track and tag them with CSS
  // overrides so the transition actually animates on the canvas.
  for (const tr of transitions) {
    const pair = transitionPairAtPlayhead(tr, clips, playhead);
    if (!pair) continue;
    const { outgoing, incoming, progress } = pair;
    const styles = transitionStyles(tr.kind, progress);

    // Ensure both clips are present in renderedItems
    const ensure = (clip: Clip, style: TransitionStyle) => {
      ...
      const item = buildItem(clip, elements, playhead);
      if (!item) return;
      renderedItems.push({ ...item, transitionStyle: style });
      includedClipIds.add(clip.id);
    };

    ensure(outgoing, styles.outgoing);
    ensure(incoming, styles.incoming);
  }

  // Paint back → front: video behind, subtitles on top (kind-based, not track id).
  renderedItems.sort((a, b) => compareClipPaintOrder(a.clip, b.clip));
...
export function logCanvasRenderPlan(plan: CanvasRenderPlan, playhead: number) {
  ...
  console.groupCollapsed(`[render-pipeline] canvas t=${playhead.toFixed(3)}`);
  // eslint-disable-next-line no-console
  console.log("Rendered", plan.renderedItems.length, "items");
  // eslint-disable-next-line no-console
  console.groupEnd();
}
