...
export class AssetRegistry {
  ...
  public async loadVideo(url: string): Promise<LoadedAsset> {
  ...
  public getAllMedia(): (HTMLVideoElement | HTMLAudioElement)[] {
    const media: (HTMLVideoElement | HTMLAudioElement)[] = [];
    for (const asset of Array.from(this.assets.values())) {
      if (asset.type === "video" || asset.type === "audio") {
        media.push(asset.element as HTMLVideoElement | HTMLAudioElement);
      }
    }
    return media;
  }
