  ...
  if (doc) {
    ...
    if ((voiceoverChanged || subtitleTrackChanged) && originalCues.length > 0) {
      ...
      if (facts && Array.isArray(facts) && !hasCustomTitles) {
        const sortedFacts = [...facts].sort((a: any, b: any) => a.startMs - b.startMs);
        for (let i = 0; i < sortedFacts.length; i++) {
          if (i > 0) {
            if (sortedFacts[i].startMs < sortedFacts[i - 1].endMs) {
              const duration = sortedFacts[i].endMs - sortedFacts[i].startMs;
              sortedFacts[i].startMs = sortedFacts[i - 1].endMs;
              sortedFacts[i].endMs = sortedFacts[i].startMs + duration;
            }
          }
        }
