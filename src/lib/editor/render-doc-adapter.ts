...
import type { EditorElement, LoadedProjectAssets } from "@/lib/editor/types";
...
import type {
  ...
  RenderTrackItem,
  RenderTransition,
} from "@/lib/editor/render-types";

const FPS = 30;
const SECONDARY_FONT = "geist-regular";
...
function buildAudioItem(el: EditorElement): RenderTrackItem | null {
  const src = resolveSrc(el);
  if (!src) return null;
  const from = el.startMs;
  const to = el.endMs;
...
function makeTrack(type: string, itemIds: string[]): RenderTrack {
  return {
    id: randomUUID(),
    type,
    items: itemIds,
...
export type ProjectRenderDocInput = {
  projectId: string;
...
/**
 * Converts project timeline + media + subtitles into a RenderDoc for the
 * Remotion export player.
 */
export function projectAssetsToRenderDoc(input: ProjectRenderDocInput): RenderDoc {
  ...
  const trackItemsMap: Record<string, RenderTrackItem> = {};
  ...
  if (loaded.timeline?.tracks) {
    for (const t of loaded.timeline.tracks) {
      ...
      if (validItems.length > 0) {
        tracks.push({
          id: t.id,
          type: t.type === "voiceover" ? "audio" : t.type,
          items: validItems,
          muted: t.muted,
        });
      }
  ...
  const transitionsMap: Record<string, RenderTransition> = {};
  ...
  if (loaded.timeline?.transitions) {
    for (const t of loaded.timeline.transitions) {
      ...
      const transitionId = t.id || `tr-${randomUUID()}`;
